...
Infrastructure for the Ordnance Survey address data acquisition pipeline that
lives in `lambdas/` in this repository.

## What the Lambda code expects

The five functions in `lambdas/` are wake-up handlers. None of them holds state
...
artefacts are deployed; all five functions are built from one Maven reactor and
...
* No Lambda module in `lambdas/` declares a shade plugin, so the published
  artefacts are thin jars. They will fail at cold start with
  `ClassNotFoundException` until the build produces an uber jar containing the
  AWS SDK `META-INF/services` entries.
...
| <a name="input_lambda_version"></a> [lambda\_version](#input\_lambda\_version) | The version of the address-lookup Lambda artefacts to deploy. All five functions are released together from the same reactor build and therefore share a version. | `string` | n/a | yes |
