...
public final class DatasetCatalog {
    ...
    public record Table(String name, String version, String physical, String key, List<Column> columns) {
        public String columnList() {
            return columns.stream().map(c -> quote(c.name())).collect(java.util.stream.Collectors.joining(","));
        }
    ...
    private static List<Table> load() {
        try (var in = DatasetCatalog.class.getResourceAsStream("/catalog.json")) {
            ...
            if (tables.size() != Dataset.values().length || tables.stream().map(Table::name).distinct().count() != tables.size())
    ...
    public static List<Table> tables() { return TABLES; }
}
