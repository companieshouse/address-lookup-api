...
public final class ReleaseStore {
    ...
    public ReleaseStore(S3Client s3) { this.s3 = s3; }
    public JsonNode read(String bucket, String key) throws Exception {
        // Prefix-scoped ListBucket grants need not reveal missing keys through GET's 404/403 distinction.
        // Check the exact permitted key before reading optional receipts.
        var existing = s3.listObjectsV2(ListObjectsV2Request.builder().bucket(bucket).prefix(key).maxKeys(1).build());
    ...
    public void immutable(String bucket, String key, JsonNode value) throws Exception {
        try {
            s3.putObject(PutObjectRequest.builder().bucket(bucket).key(key).ifNoneMatch("*").contentType("application/json").build(), RequestBody.fromString(value.toString()));
        } catch (S3Exception e) { if (e.statusCode() != 412) throw e; }
    }
    public List<String> keys(String bucket, String prefix) {
        var keys = new ArrayList<String>(); String token = null;
        do {
            var page = s3.listObjectsV2(ListObjectsV2Request.builder().bucket(bucket).prefix(prefix).continuationToken(token).build());
            page.contents().forEach(o -> keys.add(o.key())); token = page.nextContinuationToken();
        } while (token != null);
        return keys;
    }
    public String version(String bucket, String key) {
        ...
        check(head.versionId() != null && !"null".equals(head.versionId()), "Versioned bucket required");
        return head.versionId();
    }
    public String scan(String bucket, String key, String version) {
        var tags = s3.getObjectTagging(GetObjectTaggingRequest.builder().bucket(bucket).key(key).versionId(version).build());
        return tags.tagSet().stream().filter(t -> t.key().equals("GuardDutyMalwareScanStatus")).map(Tag::value).findFirst().orElse("PENDING");
    }
    public boolean clean(String bucket, JsonNode ref) {
        String key = required(ref,"key"), version = required(ref,"version");
        if (!CLEAN.equals(scan(bucket,key,version))) return false;
        check(version.equals(version(bucket,key)), "Object changed after certification: " + key);
        return true;
    }
    public static String planKey(JsonNode plan) { return "acquisitions/"+required(plan,"target")+"/"+required(plan,"runId")+".json"; }
    public static String receiptKey(String id,String dataset,String step) { return "control/"+UUID.fromString(id)+"/"+dataset+"/"+step+".json"; }
    public JsonNode receipt(String bucket,String id,String dataset,String step) throws Exception { return read(bucket,receiptKey(id,dataset,step)); }
    public void receipt(String bucket,String id,String dataset,String step,StreamingS3.Uploaded upload) throws Exception {
